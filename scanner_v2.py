...
def intraday_health(
    df_1min: pd.DataFrame, prior_close: float
    ...
    vwap = (
        float(
            (tp * today_bars["volume"]).cumsum().iloc[-1]
            / today_bars["volume"].cumsum().iloc[-1]
        )
        if today_bars["volume"].sum() > 0
        else current
    )
...
def compute_ofi(
    df_1min: pd.DataFrame, window: int = OFI_WINDOW
...
def ofi_signal(ofi: float, delta: float) -> str:
    if ofi >= 0.65 and delta >= 0:
        return "🟢 ACCUMULATING"
    elif ofi >= 0.60 and delta < 0:
        return "🟡 TOPPING"
    elif ofi <= 0.35:
        return "🔴 DISTRIBUTING"
    elif ofi <= 0.42:
...
def kelly_size(
    price: float, atr: float, win_rate: float = 0.60, rr: float = 2.0
...
def check_signal_decay(
    symbol: str, score: float, current_price: float, atr: float
...
def scan_symbol(symbol: str, market: dict) -> Optional[dict]:
    ...
    raw = float(
        np.clip(
            W_HURST * h_sc + W_HAWKES * hawk_sc + W_OFI * o_sc + W_SECTOR * sec_sc,
            0,
            100,
        )
    )
...
def run_full_scan(symbols: list = WATCHLIST) -> tuple[pd.DataFrame, dict]:
    ...
    if blocked:
        print(
            f"[FILTER] Blocked {len(blocked)} large/mega-cap(s): {', '.join(blocked)}"
        )
...
def print_results(df: pd.DataFrame, market: dict):
    ...
    print(
        f"  MARKET: {market['regime']}  SPY={market['spy_price']} ({market['spy_dev']:+.2f}%)"
    )
    print(f"{'='*80}")
    print(
        f"  {'SYM':<6} {'SCORE':>5} {'RAW':>5}  {'HEALTH':<26} {'HAWKES':<14} {'OFI':<16} {'KELLY%':>6}  {'SHR':>5}"
    )
    print(f"  {'─'*78}")
    ...
    print(f"\n  {'─'*78}")
    print(
        f"  Signals: {len(alerts)}/{len(df)}  |  Market gate: {'✓ OPEN' if market['allows_long'] else '✗ RISK-OFF'}"
    )
    ...
    if len(alerts) > 0:
        ...
        for _, r in alerts.iterrows():
            ...
            print(
                f"    Intraday: {r['intraday_ret']:+.2f}% from open | Gap: {r['gap_ret']:+.2f}% | VWAP ${r['vwap']:.2f} {'✗ BELOW' if r['below_vwap'] else '✓ ABOVE'}"
            )
            ...
            print(
                f"    Sector  : {r['sector_etf']}  RS={r['sector_rs']}  {'✓' if r['sector_gate'] else '✗'}"
            )
            print(
                f"    Size    : {r['kelly_frac']:.1%} Kelly  →  {r['shares']} shares  (${r['dollar_risk']} risk)"
            )
...
def run_dashboard():
    ...
    with col1:
        st.markdown(
            '<div class="title">⚡ QUANT SCANNER v2</div>', unsafe_allow_html=True
        )
    with col2:
        st.markdown(
    ...
    if market:
        regime_color = (
            "#00ff8c"
            if "RISK-ON" in market.get("regime", "")
            else ("#ff4040" if "RISK-OFF" in market.get("regime", "") else "#ffb400")
        )
        blocked_str = (
            f" &nbsp;|&nbsp; 🚫 {market['blocked_count']} blocked"
            if market.get("blocked_count", 0) > 0
            else ""
        )
        st.markdown(
    ...
    def render_card(r):
        ...
        sc_col = (
            "#00ff8c"
            if r["score"] >= 75
            else ("#ffb400" if r["score"] >= 60 else "#aaa")
        )
